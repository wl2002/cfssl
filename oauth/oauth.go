...
import (
	...
	"github.com/markbates/goth/providers/newProvider"
)
...
func main() {
	goth.UseProviders(
		...
		newProvider.New(os.Getenv("KEY"), os.Getenv("SECRET"), ":14000/callback"),
	)
	...
	p.Get("{provider}/callback", func(res http.ResponseWriter, req *http.Request) {
		...
		http.Redirect(res, req, "http://localhost:8888", http.StatusFound)
		//t := template.New("foo") //.Parse("127.0.0.1:8888")
		//t.Execute(res, template.URL(`localhost:8888`))
		//loadPage()
	})

	p.Get("/{provider}", gothic.BeginAuthHandler)
...
<p><a href="/newProvider"> Login </a></p>
`
